...
@Composable
internal fun ExtraCard(modifier: Modifier = Modifier) = LazyColumn(
    state = rememberLazyListState(),
    modifier = modifier.padding(vertical = 8.dp, horizontal = 16.dp),
) {
    item { Credential() }
    item { Certificate() }
    items(items = ExternalLink.entries.take(n = 4)) { FrameLink(link = it) }
}
...
private fun Certificate(
    ...
    link: ExternalLink = ExternalLink.Certificate,
